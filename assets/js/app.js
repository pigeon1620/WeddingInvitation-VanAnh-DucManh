...
// Lễ thành hôn / lễ vu quy hiển thị cả hai nhà. Tiệc cưới chỉ lấy một nhà theo config.ui.selectedEventKey.
function renderEvents() {
  const eventGrid = $("eventGrid");
  if (!eventGrid) return;

  eventGrid.innerHTML = config.events.map(event => {
    const ceremony = getCeremony(event);

    return `
      <article class="event-card">
        <img src="${escapeHtml(event.image || "")}" alt="${escapeHtml(event.imageAlt || event.label || "")}" />
        <div>
